---
title: "ECO 230: Data Analysis for Business Applications"
...
.trunc <- function(x, width) ifelse(is.na(x), x, stringr::str_trunc(x, width = width, side = "right", ellipsis = "…"))
```

## Use Instant Booking Wisely

<br> Airbnb Owners should enable **Instant Booking** in Fall — it's the unequivocally best season for the highest **rate (average price × average rating)**. <br><br>
...
season_summary %>%
  mutate(
    ...
    `Rating × Price`  = number(rating_dollars, big.mark = ",", accuracy = 0.01)
  ) %>%
  select(
    Season,
    `Bookings` = bookings,
    `Avg Price (USD)`, `Avg Rating`, `Instant-Book %`, `Rating × Price`
  ) %>%
  gt() %>%
...
##  {background-image="media/images/w01_s03_image.png" background-position="left center" background-size="auto 150%" background-repeat="no-repeat" fig-alt="A picture, a spreadsheet, or a database — all are data sources for analytics and statistics."}
...
tbl_abnb_vt <- abnb_tbl_data %>%
  ...
  cols_label(
    ...
    accommodates = "Sleeps", neighbourhood = "N’hood", bedrooms = "BR",
  ...
  tab_caption(md("**Dataset A** — Airbnb Rental Data"))
...
tbl_abnb_sim_vt <- abnb_sim_tbl_data %>%
  ...
  cols_label(
    ...
    neighbourhood = "N’hood", checkin_date = "Check‑in", nights = "Nts",
  ...
  tab_caption(md("**Dataset B** — Airbnb Rental Data"))
```
...
![](media/images/w01_s19_image.png){fig-alt="Icon emphasizing the concept of columns—variables—in a dataset." width="100%"}
:::
...
![](media/images/w01_s19_image.jpg){fig-alt="Icon emphasizing the concept of rows—observations—in a dataset." width="100%"}
:::
:::::
...
## Data Grouping: Cross-Sectional {background-image="media/images/w01_s15_image.jpeg" background-size="cover" background-opacity="0.95" fig-alt="Freeze‑frame droplet illustrating a cross‑sectional snapshot at a single point in time."}

## Data Grouping: Time-Series

![](media/images/w01_s16_image.jpeg){fig-alt="Growth sequence image suggesting change over time in a time‑series." height="85%"}

## Data Grouping: Panel

![](media/images/w01_s17_image.jpeg){.r-stretch fig-alt="Grid‑style visual suggesting repeated measures on multiple units across time (panel data)." fig-align="right" height="100%"}
...
tall_week %>%
  gt(rowname_col = "Name") %>%
  tab_header(title = md("**Tall — More rows, fewer columns**")) %>%
...
wide_week %>%
  gt() %>%
  tab_header(title = md("**Wide — More columns, fewer rows**")) %>%
  fmt_missing(everything(), missing_text = "") %>%
  gt_boland(base_size = 12, tight = FALSE) %>%
...
messy_zip_trunc <- messy_zip %>%
  mutate(
    everything_raw = ifelse(
      ...
      everything_raw,
      str_trunc(everything_raw, width = TRUNC_WIDTH, side = "right", ellipsis = "…")
    )
  )
...
![](media/images/w01_s27_image.png){fig-alt="Posit Cloud branding indicating a browser‑based RStudio environment for analysis." width="100%"}
...
## So… what actually went wrong with the Airbnb analysis?
