...
season_summary %>%
  ...
  gt() %>%
  # Minimal typography—no header, notes, or shading
...
.trunc <- function(x, width) ifelse(is.na(x), x, str_trunc(x, width = width, side = "right", ellipsis = "…"))
...
tbl_abnb_vt <-
  ...
  # Minimal borders / tight padding / small font — your VT style
  tab_options(
    table.width = pct(100),
...
tbl_abnb_sim_vt <-
  ...
  cols_label(
    ...
    guest_count = "Guests",
    status = "Status",
    price_per_night = "Price/Nt",  # no $ in header → avoids KaTeX
    total_price = "Total"           # no $ in header → avoids KaTeX
  ) %>%
...
![](media/images/w01_s17_image.jpeg){.r-stretch fig-alt="Grid‑style visual suggesting repeated measures on multiple units across time (panel data)." fig-align="right"}
...
tall_week <- sleep %>%
  ...
  group_by(Name, Gender, Location, Date, DayOfWeek) %>%
  summarise(TotalMinutes = sum(TotalMinutes), .groups = "drop") %>%
  # 🔑 CRITICAL FIX: coerce BEFORE pivot_longer
...
tall_gt <- tall_week %>%
  gt(rowname_col = "Name") %>%
  tab_header(
    title = md("**Tall — More rows, fewer columns**")
  ) %>%
  cols_label(
    Variable = "Variable",
    Value    = "Value"
  ...
  tab_options(
    ...
    # Optional: remove row separators for a “no-chrome” card look
...
wide_gt <- wide_week %>%
  gt() %>%
  tab_header(
    title = md("**Wide — More columns, fewer rows**")
  ) %>%
  fmt_missing(everything(), missing_text = "") %>%
  tab_options(
