#!/usr/bin/env python3
"""Build the local U.S. ZIP reference candidate release."""
...
SOURCE_FILES = {
    "GEOGRAPHY": "zip_cbsa_meta_2025.csv",
    "POPULATION": "zip_population_by_zip_2023.csv",
    "MEDIAN_AGE": "zip_median_age_2023.csv",
    "COMMUTE": "zip_med_commute.csv",
    "DENSITY": "zip_pop_density.csv",
    "HOUSING_AGE": "median_housing_age_by_zip_2022.csv",
    "RUCA": "zip_rucca_2010.csv",
}
ARTIFACTS = {
    "zip_geography": "us_zip_geography.parquet",
    "zip_context": "us_zip_context.parquet",
}
...
def verify_sources(source_dir: Path, manifest: dict) -> None:
    files = {details["filename"]: details for details in manifest.get("files", [])}
    if set(files) != set(SOURCE_FILES.values()):
        raise ValueError("Source manifest does not contain exactly the approved files")
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default=".data-build/zip-reference/us-zip-reference-2026-fall-v1",
    )
    ...
    connection = duckdb.connect(str(work_dir / "zip_reference.duckdb"))
    ...
    try:
        run_sql(
            ...
            **{
                placeholder: sql_path(source_dir / filename)
    ...
    (release_dir / "schemas.json").write_text(
        json.dumps(
            {name: details["schema"] for name, details in generated.items()}, indent=2
        )
        + "\n",
        encoding="utf-8",
    )
    ...
    print(f"Built ZIP reference local candidate at {release_dir}")
