...
**Current default:** 7,900 total → 7,336 with reported weather → 81 missing temperatures → **7,255 usable**.
...
```{r}
cross_counts <- with(acc_mast, table(time_period, season, useNA = "no"))
cross_pct <- 100 * prop.table(cross_counts)

cross_display <- cbind(
  cross_pct,
  Total = 100 * rowSums(cross_counts) / sum(cross_counts)
)
cross_display <- rbind(
  cross_display,
  Total = c(100 * colSums(cross_counts) / sum(cross_counts), 100)
)

as.data.frame.matrix(cross_display) %>%
  tibble::rownames_to_column("Time period") %>%
  mutate(across(-`Time period`, ~ sprintf("%.1f%%", .x))) %>%
  gt() %>%
  tab_header(
    title = "Time period by season",
    subtitle = paste0("Table percentages; usable N = ", sum(cross_counts))
  ) %>%
  tab_options(table.font.size = 13, data_row.padding = px(3))
```

[Open the interactive cross-tab app](https://shiny.60land.com/week02/crosstabs/)
...
acc_mast %>%
  ...
  group_by(season) %>%
  summarise(mean_temp = mean(temperature_f), median_temp = median(temperature_f))
```
