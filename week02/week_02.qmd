...
![](media/images/NOIR.svg){width="100%" height="700px" fig-alt="Visual representing measurement scales Nominal, Ordinal, Interval, Ratio and their common uses."}
...
[![](media/images/shiny_size.png){width="75%"}](https://shiny.60land.com/week02/descriptives/)
...
# Relationships: Discrete to Discrete {background-image="media/images/NOIR.svg" background-position="right 24px bottom 42px" background-size="480px" background-repeat="no-repeat" fig-alt="Visual representing measurement scales Nominal, Ordinal, Interval, Ratio and their common uses."}
...
# Relationships - Continuous to Continuous {background-image="media/images/NOIR.svg" background-position="right 24px bottom 42px" background-size="480px" background-repeat="no-repeat" fig-alt="Visual representing measurement scales Nominal, Ordinal, Interval, Ratio and their common uses."}
...
-   +1 means positive relationship, -1 means negative relationship
...
corr_data <- acc_mast %>%
  select(temperature_f, relative_humidity_percent) %>%
  filter(is.finite(temperature_f), is.finite(relative_humidity_percent))

ggplot(corr_data, aes(temperature_f, relative_humidity_percent)) +
  geom_point(alpha = 0.18, size = 0.7, color = "#4C78A8") +
  geom_smooth(method = "lm", se = FALSE, color = "black", linewidth = 0.8) +
  labs(
    x = "Temperature (F)",
    y = "Relative humidity (%)",
    subtitle = paste0(
      "Complete pairs: n = ", scales::comma(nrow(corr_data)),
      " | Pearson r = ",
      sprintf("%.3f", cor(corr_data$temperature_f, corr_data$relative_humidity_percent))
    )
  ) +
  theme_minimal(base_size = 12)
```

[Open the interactive correlation app](https://shiny.60land.com/week02/correlation/)
:::
::::
:::::

# Relationships: Discrete to Continuous {background-image="media/images/NOIR.svg" background-position="right 24px bottom 42px" background-size="480px" background-repeat="no-repeat" fig-alt="Visual representing measurement scales Nominal, Ordinal, Interval, Ratio and their common uses."}
